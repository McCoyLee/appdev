...
- **M0~M3 全部完成**（26 个任务）+ **M4-1~M4-10**（PR 协作闭环 + diff + 行内评论 + review + CI 徽章 / PWA / agent PR 工具 / 拆包 / 单测）已做
- 后端 28 个端点（+`/api/prs` 9 个）/ AI 18 个工具（+list_prs/comment_pr/read_pr_feedback/merge_pr）/ 前端 13 个组件（+PrPanel）/ 6 个模板 / Tauri 骨架（未编译）/ PWA（manifest+sw，可安装）
...
│   ├── prs.py              # M4 PR 协作：列表/详情(文件+CI+评论+行内+reviews)/checks/开/留言/行内评论/review/合并/关闭
